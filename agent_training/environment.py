...
# scale_torque_norm_thruster = np.sqrt(constants['u_max_yang']**2 + constants['u_max_yang']**2 + constants['u_max_yang']**2)  # For Yang's environment with thrusters
scale_torque_norm_thruster = np.sqrt(constants['u_max']**2 + constants['u_max']**2 + constants['u_max']**2)  # For Yang's environment with thrusters
...
def reward_function_thruster(state, torque, torque_prev):
    ...
    if USE_REWARD == "yang":
        r_err = np.exp(-err_phi_current/(0.14*360))
        r_torque = -0.05 * np.sqrt(torque_1**2 + torque_2**2 + torque_3**2)/scale_torque_norm_thruster - 0.005 * (np.sqrt((torque_1-torque_1_prev)**2 + (torque_2-torque_2_prev)**2 + (torque_3-torque_3_prev)**2))
        r_acc = 0
        if err_phi_current < 0.25:
            r_acc = 9
...
class SatThrusterEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.state = np.concatenate((q_array_initial, omega_initial, [q0_prev]))
    ...
    def step(self, action):
        ...
        q0_prev = self.state[0]     # store current q0 before integration
        self.torque_prev = action * constants["u_max_yang"]  # store current torque before integration
        #self.torque_prev = action * constants["u_max"]  # store current torque before integration
